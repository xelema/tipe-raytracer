...
int main(){
    ...
    for (int frame = 1; frame <= 47; frame++){
        ...
        point3 origin = {{-0.7+posX, 0.2-posY, 0.2-posZ}}; // position de la camera
        point3 target = {{0.6-targetX, -0.5, -3.3}}; // cible de la camera
        vec3 up = {{0, 1, 0}}; // permet de modifier la rotation selon l'axe z ({{0, 1, 0}} pour horizontal)

        //qualité et performance
        int nbRayonParPixel = 600;
        int nbRebondMax = 6;
        ...
        sphere h_sphere_list[11] = {
            ...
            {{{-501,0,0}}, 500, {GREEN, BLACK, 0.0, 0.95}},
            {{{0,-501,0}}, 500, {WHITE, BLACK, 0.0, 0.0}},
            {{{501, 0, 0}}, 500, {RED, BLACK, 0.0, 0.95}},
            {{{-0.5, -1.4+lightY, -3}}, 0.5, {BLACK, {{1.0, 0.6, 0.2}}, 4.0, 0.0}},
            {{{0.5, -1.4+lightY, -2.2}}, 0.5, {BLACK, {{0.7, 0.2, 1.0}}, 4.0, 0.0}},
            {{{-0.5, 1.4-lightY, -1.5}}, 0.5, {BLACK, {{0.55, 0.863, 1.0}}, 2.5, 0.0}},
            {{{0.5, 1.4-lightY, -3.1}}, 0.5, {BLACK, {{0.431, 1.0, 0.596}}, 2.5, 0.0}},
            {{{0, 0, -504}}, 500, {WHITE, BLACK, 0.0, 0.0}},
            {{{0, 501, 0}}, 500, {WHITE, BLACK, 0.0, 0.0}},
            {{{-0.4, -0.5, -3.3}}, 0.5, {SKY, BLACK, 0.0, 1.0}},
            {{{0.2, -0.7, -1.6}}, 0.3, {SKY, BLACK, 0.0, 0.5}},
        };
        // nom du fichier
        char nomFichier[100];
        ...
        sprintf(nomFichier, "%d_ANIMATION_%dRAYS_%dRB_%02d-%02d_%02dh%02d.ppm", frame, nbRayonParPixel, nbRebondMax-1, temps->tm_mday, temps->tm_mon + 1, temps->tm_hour, temps->tm_min);
    ...
    for (int frame = 47; frame <= 59; frame++){
        // CONSTANTES (paramètres de rendu)
        ////////////////////////////////////////////////////////
        ////////////////////////////////////////////////////////
    ...
    int minutesTotales = (int)(tempsTotal / 60000);
    int secondsTotales = (int)((tempsTotal - minutesTotales * 60000) / 1000);
    fprintf(stderr, "Temps de rendu total : %d min %d sec\n", minutesTotales, secondsTotales);
